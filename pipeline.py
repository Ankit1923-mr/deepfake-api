...
def word_to_openness(word):
    try:
        ...
        phon = phonemize(
            word.strip().lower(), backend="espeak",
    ...
    for segment in whisper_result["segments"]:
        for word_info in segment.get("words", []):
            ...
            for j in range(n_frames):
                idx = min(int(j / n_frames * len(openness_seq)), len(openness_seq)-1)
...
def detect_fake_segments(window_scores, threshold=0.45):
    if not window_scores:
        return [], 0.0
    flagged = [(s["start"], s["end"], s["combined"])
...
# ── Feature extraction ───────────────────────────────────────────
def extract_features(lip_curve, phoneme_curve, fps, optimal_threshold=0.45):
    lip_s = smooth_lip_curve(lip_curve).astype(np.double)
    ph    = phoneme_curve.astype(np.double)
    ...
    xcorr      = correlate(lip_s - lip_s.mean(), ph - ph.mean(), mode="full")
    xcorr_peak = np.max(np.abs(xcorr)) / (n * (lip_s.std() * ph.std() + 1e-8))
    ...
    lip_peaks, _ = find_peaks(lip_s, height=0.3, distance=int(fps * 0.2))
    ph_peaks,  _ = find_peaks(ph,    height=0.4, distance=int(fps * 0.2))
    lip_peak_rate = len(lip_peaks) / duration if duration > 0 else 0
    ph_peak_rate  = len(ph_peaks)  / duration if duration > 0 else 0
    peak_ratio    = float(np.clip(lip_peak_rate / (ph_peak_rate + 1e-8), 0, 50))
    ...
    window_scores = sliding_window_dtw_scores_v2(lip_curve, phoneme_curve, fps)
    if window_scores:
        ...
        segs, fake_ratio = detect_fake_segments(window_scores, threshold=optimal_threshold)
...
def run_pipeline(video_path, model_path, rf_model, scaler,
    ...
    Returns a result dict with prediction, confidence, and fake segments.
    """
    with load_landmarker(model_path) as landmarker:
        lip_curve, fps, frame_count = extract_lip_curve(video_path, landmarker)
    ...
    try:
        ...
        result = whisper_model.transcribe(tmp_wav, word_timestamps=True, language="en")
    finally:
        if os.path.exists(tmp_wav):
            os.remove(tmp_wav)

    has_words = any(len(s.get("words", [])) > 0 for s in result["segments"])
    phoneme_curve = (words_to_phoneme_curve(result, frame_count, fps)
                     if has_words else np.zeros(frame_count, dtype=np.float32))

    features = extract_features(lip_curve, phoneme_curve, fps, optimal_threshold)
    feature_order = [
        "pearson","xcorr_peak","lip_variance","lip_mad","lip_range",
        "phoneme_coverage","peak_ratio","mad_global","mismatch_open_closed",
        "mismatch_closed_open","mean_dtw","max_dtw","std_dtw","mean_corr",
        "mean_combined","std_combined","fake_ratio","n_fake_segments"
    ]
